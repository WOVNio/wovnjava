...
class FileExtensionMatcher {
    ...
    /*
     * Warning! Java 7 has a dangerous bug that may cause complex regexes to fail
     * https://stackoverflow.com/questions/14712932/bug-in-java-regex-implementation
     */
    FileExtensionMatcher() {
        ...
        this.DOC_FILE_PATTERN = Pattern.compile("(\\.(gzip|7zip|zip|7z|tar|gz|rar|ez|aw|atom(cat|svc)?|(cc)?xa?ml|cdmi(a|c|d|o|q)?|epub|g(ml|px|xf)|jar|js|ser|class|json(ml)?|do(c|t)m?|xps|pp(a|tx?|s)m?|potm?|sldm|mp(p|t)|bin|dms|lrf|mar|so|dist|distz|m?pkg|bpk|dump|rtf|tfi|pdf|pgp|apk|o(t|d)(b|c|ft?|g|h|i|p|s|t)))$");
    }
